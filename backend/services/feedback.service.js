...
module.exports.updateFeedback = async (id, feedbackData) => {
  const feedback = await feedbackModel.findByIdAndUpdate(id, feedbackData, {
    new: true,
    runValidators: true,
  });
  if (!feedback) {
...
module.exports.toogleFeedbackApproval = async (id, is_approved) => {
  const updateFeedback = await feedbackModel.findByIdAndUpdate(
    id,
    { is_approved },
    { new: true }
  );
...
module.exports.getApprovedFeedbacks = async () => {
  const approvedFeedbacks = await feedbackModel.find({ is_approved: true }).sort({ submitted_at: -1 });
